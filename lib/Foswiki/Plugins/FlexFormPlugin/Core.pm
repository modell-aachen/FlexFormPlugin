...
sub handleRENDERFORDISPLAY {
    ...
    if ( !defined($theFormat) && !defined($theHeader) && !defined($theFooter) )
    {
        ...
        $theHeader =
'<div class=\'foswikiFormSteps\'><table class=\'foswikiLayoutTable\'>';
        $theFooter = '</table></div>';
        $theFormat = '<tr>
    ...
    $theForm = $session->{request}->param('formtemplate')
      unless defined $theForm;
    ...
    ( $theFormWeb, $theForm ) =
      Foswiki::Func::normalizeWebTopicName( $theFormWeb, $theForm );
    ...
    if ($theFields) {
        foreach my $fieldName ( split( /\s*,\s*/, $theFields ) ) {
            ...
            writeDebug(
                "WARNING: no field for '$fieldName' in $theFormWeb.$theForm")
              unless $field;
...
# CAUTION: don't use field->getOptions() on a +values field as that won't return the full valueMap...only the value part, but not the title map
        ...
        $fieldTitle = $params->{ $fieldName . '_title' }
          if defined $params->{ $fieldName . '_title' };    # see also map
        ...
        $fieldValue = $params->{ $fieldName . '_value' }
          if defined $params->{ $fieldName . '_value' };
        ...
        my $fieldSort =
          Foswiki::Func::isTrue( $params->{ $fieldName . '_sort' }, $theSort );
        $fieldAllowedValues = sortValues( $fieldAllowedValues, $fieldSort )
          if $fieldSort;
        ...
        $fieldTitle = $fieldTitles->{$fieldName}
          if $fieldTitles && $fieldTitles->{$fieldName};
...
# we mimic the core behaviour here
    ...
    $session->{plugins}
      ->dispatch( 'beforeEditHandler', $text, $thisTopic, $thisWeb, $topicObj );
    $topicObj->text($text);
...
# CAUTION: don't use field->getOptions() on a +values field as that won't return the full valueMap...only the value part, but not the title map
  ...
  # fallback to field->value
        ...
        if ( $field->can('getOptions') ) {
            ...
            writeDebug(
                "can't getOptions ... fallback to field->{value} for $fieldName"
            );
        ...
        $fieldEdit =
          $session->{plugins}->dispatch( 'renderFormFieldForEditHandler',
            $fieldName, $fieldType, $fieldSize, $fieldValue, $fieldAttrs,
            $fieldAllowedValues );
        ...
        unless ($fieldEdit) {
            if ($isHidden) {
                ...
                if ( $Foswiki::Plugins::VERSION > 2.0 ) {
                    ...
                    # pre-TOM
                    ( $fieldExtra, $fieldEdit ) =
                      $field->renderForEdit( $thisWeb, $thisTopic,
                        $fieldValue );
                }
            }
        }
